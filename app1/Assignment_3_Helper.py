...
import faulthandler
faulthandler.enable()
from icfg_visualizer import create_icfg_visualization, create_unvisited_icfg_visualization, create_scc_icfg_visualization
import os
import pysvf
...
class AbstractExecutionHelper:
    ...
    def printReport(self):
        logging.info("Total checkpoint: " + str(self.checkpoint_count))
        if len(self.node_to_bug_info) > 0:
            logging.info("######################Buffer Overflow ({} found)######################".format(len(self.node_to_bug_info)))
            logging.info("---------------------------------------------")
            for node, msg in self.node_to_bug_info.items():
                logging.info(f"{node}: {msg}\n---------------------------------------------")
        else:
            logging.info("No buffer overflow found")
    ...
    def getStrlen(self, abstractState, strValue):
        ...
        if length == 0:
            # temp 512 is the max length of a string
            return pysvf.IntervalValue(0, 512)
        else:
...
class AbstractExecution:
    ...
    def initWto(self):
        ...
        self.svfir.getCallGraph().dump("call_graph")
    ...
    def handleWtoComponents(self, wtoComps, context_depth: int):
        for comp in wtoComps:
            if isinstance(comp, ICFGWTONode):
                self.handleSingletonWto(comp, context_depth)
            elif isinstance(comp, ICFGWTOCycle):
                self.handleCycleWto(comp, context_depth)
    ...
    def handleCycleWto(self, cycle: ICFGWTOCycle, context_depth: int):
        ...
        if is_feasible is False:
            ...
            self.pre_abs_trace[cycle.head.node] = tmp_es
        pre_as = self.pre_abs_trace[cycle.head.node]
        self.post_abs_trace[cycle.head.node] = AbstractState
        increasing = True
        iteration = 0
        while True:
            previosu_as = self.post_abs_trace[cycle.head.node]
            self.handleSingletonWto(cycle.head, context_depth)
            current_as = self.post_abs_trace[cycle.head.node]
            if iteration >= self.widen_delay:
                if increasing:
                    self.post_abs_trace[cycle.head.node] = previosu_as.widening(current_as)
                else:
                    self.post_abs_trace[cycle.head.node] = previosu_as.narrowing(current_as)
                if self.post_abs_trace[cycle.head.node] == previosu_as :
                    if increasing:
                        increasing = False
                        continue
                    else:
                        break
            self.handleWtoComponents(cycle.components, context_depth)
            iteration += 1

    def getPAGSource(self, stmt: pysvf.SVFStmt) -> str:
        res = str(stmt)
        worklist = [stmt]
        visitedNodes = set[pysvf.SVFStmt]()
        ...
        while worklist:
            ...
            if isinstance(stmt, pysvf.AddrStmt):
                res += str(stmt)
                srcNodes.append(stmt.getRHSVar())
            if isinstance(stmt, pysvf.BinaryOPStmt):
                res += str(stmt)
                srcNodes.append(stmt.getOpVar(0))
                srcNodes.append(stmt.getOpVar(1))
            if isinstance(stmt, pysvf.CmpStmt):
                res += str(stmt)
                srcNodes.append(stmt.getOpVar(0))
                srcNodes.append(stmt.getOpVar(1))
            if isinstance(stmt, pysvf.LoadStmt):
                res += str(stmt)
                srcNodes.append(stmt.getRHSVar())
            if isinstance(stmt, pysvf.StoreStmt):
                res += str(stmt)
                srcNodes.append(stmt.getRHSVar())
            if isinstance(stmt, pysvf.CopyStmt):
                res += str(stmt)
                srcNodes.append(stmt.getRHSVar())
            if isinstance(stmt, pysvf.GepStmt):
                res += str(stmt)
                srcNodes.append(stmt.getRHSVar())
            if isinstance(stmt, pysvf.PhiStmt):
                res += str(stmt)
                for opndVar in stmt.getOpndVars():
                    srcNodes.append(opndVar)
            if isinstance(stmt, pysvf.CallPE):
                res += str(stmt)
                srcNodes.append(stmt.getRHSVar())
            if isinstance(stmt, pysvf.RetPE):
                res += str(stmt)
                srcNodes.append(stmt.getRHSVar())
            if isinstance(stmt, pysvf.SelectStmt):
                res += str(stmt)
                # for opndVar in stmt.getOpndVars():
                #     srcNodes.append(opndVar)
            if isinstance(stmt, pysvf.UnaryOPStmt):
                res += str(stmt)
                srcNodes.append(stmt.getOpVar(0))
            ...
            # if isinstance(stmt, pysvf.AddrStmt): # AddrStmt and subclass (Load/Store)
            #     srcNode = stmt.getRHSVar()
            # else:
            #     srcNode = stmt.getSrcNode()
            for srcNode in srcNodes:
                if isinstance(srcNode, pysvf.GlobalICFGNode):
                    continue
                visitedNodes.add(srcNode.getId())
                for edge in srcNode.getInEdges():
    ...
    """
    def handleSingletonWto(self, singleWto: ICFGWTONode, context_depth: int):
        node = singleWto.node
        # if node.getFun().getName() == "main":
        #     print(f"handleSingletonWto: {node.getId()}")
        if context_depth == 0:
            self.post_abs_trace[node] = self.post_abs_trace[self.icfg.getGlobalICFGNode()]
        else:
            is_feasible, tmp_es, valid_predecessors = self.mergeStatesFromPredecessors(node)
            if node.getId() == 390:
                stmt = node.getSVFStmts()[0]
                assert isinstance(stmt, pysvf.StoreStmt)
                print(self.getPAGSource(stmt))
            if is_feasible:
                self.pre_abs_trace[node] = tmp_es
                self.post_abs_trace[node] = self.pre_abs_trace[node]
            else:
                return

        self.visited_node.add(node.getId())
        for stmt in node.getSVFStmts():
            self.updateAbsState(stmt)
            if not isinstance(node, pysvf.CallICFGNode):
                self.buf_overflow_helper.checkpoint_count += 1
                self.bufOverflowDetection(stmt)

        if isinstance(node, pysvf.CallICFGNode):
            if node.getCalledFunction() is None:
                return
            ...
            if fun_name == 'OVERFLOW' or fun_name == 'svf_assert' or fun_name == 'svf_assert_eq':
                ...
                self.handleCallSite(node, context_depth)
    ...
    def handleFun(self, fun: pysvf.FunObjVar, context_depth: int):
        if context_depth >= 3:
            logging.info("context_depth >= 3 exit")
            return False
        logging.info(f"handleFun: {fun.getName()}")
        wto = self.func_to_wto[fun]
        self.handleWtoComponents(wto.components, context_depth)
        return True
        

    
    def handleFunction(self, funEntry: pysvf.ICFGNode, context_depth: int):
        if context_depth >= 3:
            return

        logging.info(f"wto of {funEntry.getFun().getName()}: {self.getFunctionWto(funEntry.getFun())}")
        if funEntry.getFun().getName() == "fopen_safer":
            create_icfg_visualization(self.icfg, funEntry.getFun().getName(), "fopen_safer_icfg")
        ...
        while worklist:
            ...
            if node in self.cycle_head_to_cycle:
                ...
                self.handleICFGCycle(cycle, context_depth)
                ...
                if not self.handleICFGNode(node, context_depth):
                    logging.debug(f"Fixpoint reached or infeasible for node {str(node)}")
                    continue
    ...
    """
    def handleICFGNode(self, node: pysvf.ICFGNode, context_depth: int):
        self.visited_node.add(node.getId())
        if node in self.pre_abs_trace:
            pass
        else:
            is_feasible, self.pre_abs_trace[node] = self.mergeStatesFromPredecessors(node)

            if not is_feasible:
                logging.debug(f"Infeasible for node {node.getId()}")
                return False
        logging.debug(f"feasible for node {node.toString()}")
        ...
        self.post_abs_trace[node] = self.pre_abs_trace[node]
        assert self.post_abs_trace[node] is not None
    ...
    return void
    """
    def handleCallSite(self, node: pysvf.CallICFGNode, context_depth: int):
        fun_name = node.getCalledFunction().getName()
        logging.debug(f"handleCallSite: {fun_name}")
        if fun_name == "OVERFLOW" or fun_name == "svf_assert" or fun_name == "svf_assert_eq":
            self.handleStubFunction(node)
            ...
            if node.getRetICFGNode().getActualRet():
                lhs_id = node.getRetICFGNode().getActualRet().getId()
                self.post_abs_trace[node][lhs_id] = AbstractValue(IntervalValue.top())
            pass
            ...
            analysis_flag = self.handleFun(node.getCalledFunction(), context_depth + 1)
            if analysis_flag is False:
                # get return site and set top
                return_site = node.getRetICFGNode()
                if return_site.getActualRet():
                    lhs_id = return_site.getActualRet().getId()
                    self.post_abs_trace[return_site] = self.pre_abs_trace[node]
                    self.post_abs_trace[return_site][lhs_id] = AbstractValue(IntervalValue.top())
                return 
            return 
    ...
    def handleStubFunction(self, callNode: pysvf.CallICFGNode):
        ...
        if callNode.getCalledFunction().getName() == "svf_assert":
            ...
            if abstract_state[arg0].getInterval().isTop():
                logging.warning(f"svf_assert Fail. {callNode}")
                assert False
            else:
                if (abstract_state[arg0].getInterval().equals(IntervalValue(1, 1)) or
                        abstract_state[arg0].getInterval().equals(IntervalValue(-1, -1))):
                    logging.info(f"The assertion ({callNode}) is successfully verified!!")
                else:
                    logging.warning(f"The assertion ({callNode}) is unsatisfiable!!")
                    assert False
            ...
            if gep_rhs_val.isAddr():
                ...
                for addr in gep_rhs_val.getAddrs():
                    ...
                    if self.buf_overflow_helper.hasGepObjOffsetFromBase(gep_lhs_obj_var):
                        ...
                        if overflow:
                            logging.info("obj len: {}, you want to access: {}.".format(size, access_offset))
                    else:
                        raise AssertionError("Pointer not found in gepObjOffsetFromBase")

                if overflow:
                    logging.info("Your implementation successfully detected the buffer overflow")
                else:
                    logging.warning(f"Your implementation failed to detect the buffer overflow! {callNode}")
                    assert False
            else:
                logging.warning(f"Your implementation failed to detect the buffer overflow! {callNode}")
                assert False
    ...
    def mergeStatesFromPredecessors(self, block: pysvf.ICFGNode):
        ...
        for edge in block.getInEdges():
            if edge.getSrcNode() in self.post_abs_trace:
                if isinstance(edge, pysvf.IntraCFGEdge):
                    if edge.getCondition():
                        ...
                        if self.isBranchFeasible(edge, tmp_es):
                            ...
                            in_edge_num += 1
                            valid_predecessors.append(edge.getSrcNode().getId())
                        else:
                            pass
                    else:
                ...
                pass
        if in_edge_num == 0:
            if block in self.pre_abs_trace:
                return (True, self.pre_abs_trace[block], [])
            else:
                logging.debug(f"Error: No predecessors for block {block.getId()}")
                return (False, None, [])
        return (True, abstract_state, valid_predecessors)
    ...
    def analyse(self):
        ...
        self.internal_function = set()
        self.svfir.getCallGraph().dump("call_graph.dot")
        

        scc_list = self.getCallGraphScc()
        # print scc_list
        for scc in scc_list:
            print("scc: ", scc)
        self.key_of_call_graph_scc = self.getCallGraphScc()

        self.scc_to_icfg_node = self.splitICFGNodeBySCC(self.icfg, scc_list)

        self.initWto()
        self.handleGlobalNode()

        for call_node in self.svfir.getCallGraph().getNodes():
            fun = call_node.getFunction()
            for i in range(fun.arg_size()):
                arg = fun.getArg(i)
                as_state = self.pre_abs_trace[self.icfg.getGlobalICFGNode()]
                as_state[arg.getId()] = IntervalValue.top()

            if pysvf.isExtCall(fun):
                continue
            # temp:
            # if not fun.getName() == "main":
            #     continue
            logging.info(f"Analyse fun: {call_node.toString()}")
            self.internal_function.add(fun)
            fun_entry = self.svfir.getICFG().getFunEntryICFGNode(fun)
        ...
        for fun in self.external_funs:
            logging.info(f"External function: {str(fun)}")

        logging.info(f"Coverage: {len(self.visited_node)} / {len(self.svfir.getICFG().getNodes())}")

        # print fun name of external functions
        for fun in self.external_funs:
            logging.info(f"External function: {fun.getName()}")
    ...
    def updateStateOnCmp(self, cmp: pysvf.CmpStmt):
        ...
        elif abstract_state.getVar(op0.getId()).isAddr() and abstract_state.getVar(op0.getId()).isAddr():
            res_val = None
    ...
    def dumpUnvisited(self):
        folder  = "unvisited"
        if not os.path.exists(folder):
            os.makedirs(folder)
        for scc, icfg_nodes in self.scc_to_icfg_node.items():
            visited_num, total_num = create_scc_icfg_visualization(self.icfg, scc.split(","), icfg_nodes, self.visited_node)
            if total_num > 0:
                print(f"{scc}: visited {visited_num} / {total_num} nodes, ratio: {visited_num/total_num:.2f}")
                self.scc_coverage[scc] = visited_num / total_num

        # rank scc by coverage
        self.scc_coverage = sorted(self.scc_coverage.items(), key=lambda x: x[1], reverse=False)
        print("Ranked SCC by coverage:")
        for scc, ratio in self.scc_coverage:
            print(f"{scc}: {ratio}")

    # TODO:

    def dumpUnvisitedScc(self, fun_name: str):
        scc = self.fun_name_to_scc[fun_name]
        scc_name = ",".join(scc)
        icfg_nodes = self.scc_to_icfg_node[scc_name]
        visited_num, total_num = create_scc_icfg_visualization(self.icfg, scc, icfg_nodes, self.visited_node)
        if total_num > 0:
            print(f"{scc}: visited {visited_num} / {total_num} nodes, ratio: {visited_num/total_num:.2f}")
            self.scc_coverage[scc_name] = visited_num / total_num



    def getCallGraphScc(self) -> dict: # key is the name of the function, value is the scc of the function
        callgraph = self.svfir.getCallGraph()
        # Tarjan's SCC algorithm over call graph
        nodes = list(callgraph.getNodes())
        ...
        def get_node_name(node):
            # Prefer CallGraphNode.getName() if available, otherwise fallback to function name
            try:
                return node.getName()
            except Exception:
                try:
                    fun = node.getFunction()
                    return fun.getName() if fun else str(node.getId())
                except Exception:
                    try:
        ...
        sccs: List[List[str]] = []
        for comp in sccs_nodes:
            names = [get_node_name(n) for n in comp]
            sccs.append(names)

        return sccs

    def splitICFGNodeBySCC(self, icfg: pysvf.ICFG, sccs: List[List[str]]):
        # map from function name to scc
        self.fun_name_to_scc = {}

        for scc in sccs:
            for name in scc:
                self.fun_name_to_scc[name] = scc

        dict_scc = {}
        for scc in sccs:
            # scc to list of function names "funA,funB,funC"
            dict_scc[",".join(scc)] = []
        
        # key: List[str]
        # value: List[pysvf.ICFGNode]
        # return a dict, key is the name of the function, value is the list of icfg nodes in the scc
        
        for node in icfg.getNodes():
            if node.getFun() is not None:
                fun_name = node.getFun().getName()
                if fun_name in self.fun_name_to_scc:
                    scc = self.fun_name_to_scc[fun_name]
                    dict_scc[",".join(scc)].append(node)
        return dict_scc
